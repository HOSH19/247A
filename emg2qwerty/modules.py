...
class TransformerEncoder(nn.Module):
    """Transformer encoder for sequence modeling. Input and output have the same
    temporal length (no downsampling). Expects input of shape (T, N, num_features).

    Args:
        d_model (int): Model dimension (must match input feature size or use
            in_proj to project).
        nhead (int): Number of attention heads.
        num_encoder_layers (int): Number of transformer encoder layers.
        dim_feedforward (int): Dimension of feedforward network.
        dropout (float): Dropout probability.
        in_features (int): Input feature dimension. If different from d_model,
    ...
    def __init__(
        ...
        self.in_proj = None
        if in_features is not None and in_features != d_model:
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            activation="gelu",
            batch_first=False,  # (T, N, d_model) - time first for CTC compatibility
            norm_first=False,
        )
